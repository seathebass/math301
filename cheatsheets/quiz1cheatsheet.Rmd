...
```
where x = number of successes specified, size = total number of trials, prob = probability associated with the outcome
...
```
where x = number of successes specified, m = number of possible successes, n = number of possible failures, k = subset of the total number of outcomes
...
qpois() #quantile
```
where x = number of occurrences we are interested in and lambda = average number per time period selected.
