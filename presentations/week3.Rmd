...
library(tidyverse)
studentdata <- read_csv("~/mat301/data/studentmajors.csv")
```
...
- From the prior slide, note that a variable which can only assume **distinct** values is said to be discrete. Typically, they represent counts.
  ...
  - From the prior slide a dicrete random variable could manifest itself as a bernoulli experiment, a binomial, or a geometric variable.
...
- Simply put, a continuous random variable can assume any value in a finite or infinite interval is said to be continuous.
  - In slide 2, these would typically take the form of measurements

---
## Examples
- Discrete Variable examples:
  - A dog is selected at random from a shelter with male {M} and female {F} dogs. The sample space would then be 
  $$S = \{M,F\}$$ 
...
ggplot(pmfcdf, aes(value, cdf))+
  stat_ecdf() + ##note we won't worry too much about the differences between ecdf and cdfs yet. 
